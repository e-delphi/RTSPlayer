...
type
  ...
  strict private
    ...
    FFrameSeq: Int64;     // conta os quadros entregues (sob FFrameLock)
    // O pts do quadro que esta em FFrameRGBA, em microssegundos, como veio do
    // MediaCodec. E o unico jeito de saber A QUE instante o quadro pronto
    // corresponde: entre a entrada e a saida ha uma fila, e quem alimenta nao
    // pode supor que saiu o que acabou de entrar.
    FFramePtsUs: Int64;   // sob FFrameLock
    FConvPtsUs: Int64;    // o do buffer sendo convertido (thread do decoder)
    FMaxDisplayDim: Integer;
  ...
  public
    ...
    // Quantos quadros ja sairam. Serve para saber que o quadro mudou sem
    // depender do OnFrame, que passa pelo TThread.Queue e portanto pela thread
    // principal -- quem le isto e uma conexao do servidor local, que nao pode
    // ficar refem dela.
    function FrameSeq: Int64;
    // O instante do quadro atual, em microssegundos, no mesmo relogio dos pts
    // que foram enfileirados. Ler entre LockLatestFrame e UnlockFrame.
    function FramePtsUs: Int64;
    // Maior lado do quadro entregue; acima disso ele e reduzido. O padrao
    // atende a exibicao na tela do aparelho; quem manda a imagem para outro
    // lugar pode querer mais.
    property MaxDisplayDim: Integer read FMaxDisplayDim write FMaxDisplayDim;
...
begin
  ...
  FMaxDisplayDim := MAX_DISPLAY_DIM;
...
var
  Img: JImage;
begin
  // Guardado antes de qualquer outra coisa: o FBufferInfo e reusado a cada
  // dequeue, e o valor tem de acompanhar ESTE quadro ate o fim da conversao.
  FConvPtsUs := FBufferInfo.presentationTimeUs;
...
begin
  ...
  if LongSide > FMaxDisplayDim then
  begin
    DW := Integer((Int64(W) * FMaxDisplayDim) div LongSide);
    DH := Integer((Int64(H) * FMaxDisplayDim) div LongSide);
    if DW < 1 then DW := 1;
    if DH < 1 then DH := 1;
  end
  else
...
function TVideoDecoder.FramePtsUs: Int64;
begin
  // Sem lock proprio: e para ser lido com o quadro ja travado.
  Result := FFramePtsUs;
end;

function TVideoDecoder.FrameSeq: Int64;
begin
  FFrameLock.Enter;
  try
    Result := FFrameSeq;
